...
export const MapCanvas: React.FC<MapCanvasProps> = ({
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      ctx.fillStyle = "#0f1626"; // Dark teal-blue sea
      ...
      for (let x = 0; x < 800; x += 60) {
        for (let y = 0; y < 600; y += 60) {
          const waveShift = Math.floor(frameTick / 18) % 4;
          if ((x + y) % 120 === 0) {
            ctx.fillRect(x + waveShift * 2, y, 8, 2);
            ctx.fillRect(x + waveShift * 2 + 3, y + 2, 4, 1);
          }
        }
      }
      ...
      // Beach Shoreline
      ctx.fillStyle = "#701a75"; // Vibrant deep violet shore
      ctx.beginPath();
      ctx.arc(170, 200, 150, 0, Math.PI * 2);
      ctx.fill();

      // Main Landmass
      ctx.fillStyle = "#3b0764"; // Deep amethyst purple soil
      ctx.beginPath();
      ctx.arc(170, 200, 142, 0, Math.PI * 2);
      ctx.fill();

      // Futuristic floor neon energy rings
      ctx.strokeStyle = "rgba(168, 85, 247, 0.35)";
      ctx.lineWidth = 1.5;
      ctx.beginPath();
      ctx.arc(170, 200, 110, 0, Math.PI * 2);
      ctx.stroke();
      ...
      // Cyber Math houses (Spires of logical reasoning)
      ...
      // Sandy shoreline
      ctx.fillStyle = "#fef08a"; // Beautiful bright Yellow Stardew shore
      ctx.beginPath();
      ...
      // Draw Stardew dirt patches (fallow fields for crops)
      ...
      // 6. DRAW ALL INTERACTIVE AND GLOWING LANDMARK NODES ON EARTH GRID
      ...
      // 7. DRAW CLICK-DESTINATION FLAG (if exists and moving)
      if (isWalking) {
      ...
      // 8. DRAW JRPG PLAYER AVATAR (Voyager character with pixel coat and headband)
  ...
  }, [solvedNodeIds, activeNodeId, isDialogueOpen, direction, isWalking, hoveredNode]);

  return (
    <div className="relative border-4 border-slate-700 bg-slate-950 rounded-xl overflow-hidden shadow-2xl">
      {/* Dynamic Header Overlay */}
      <div className="absolute top-0 inset-x-0 bg-slate-900/95 border-b border-slate-800 text-xs text-slate-300 py-2 px-3 flex justify-between items-center z-10 font-mono select-none">
        ...
        <div>
          {isDialogueOpen ? (
            <span className="text-amber-400">SOCRATIC DIALOGUE ACTIVE</span>
          ) : (
            <span className="text-slate-400">CLICK WORLD TO NAVIGATE AVATAR</span>
          )}
        </div>
        <div className="text-slate-400">
          X: {Math.round(avatarPos.x)} | Y: {Math.round(avatarPos.y)}
        </div>
      </div>
      ...
      <canvas
        ...
        className="w-full aspect-[4/3] block bg-slate-950"
      />

      {/* Hover Tooltip Overlay */}
      {hoveredNode && !isDialogueOpen && (
        <div
          className="absolute font-mono select-none pointer-events-none p-3 bg-slate-900/95 border border-slate-700 text-white rounded shadow-xl text-xs flex flex-col space-y-1"
          style={{
            left: `${Math.min(590, (hoveredNode.x / 800) * 100)}%`,
            top: `${Math.max(5, (hoveredNode.y / 600) * 100 - 18)}%`,
            maxWidth: "240px",
            zIndex: 30,
          }}
        >
          <div className="flex justify-between items-center bg-slate-800 px-1.5 py-0.5 rounded text-[10px] text-white">
            <span className="uppercase tracking-wider">{hoveredNode.discipline}</span>
            <span className={solvedNodeIds.includes(hoveredNode.id) ? "text-emerald-400" : "text-amber-400"}>
              {solvedNodeIds.includes(hoveredNode.id) ? "● MASTERED" : "○ UNEXPLORED"}
            </span>
          </div>
          <span className="font-bold text-sm text-yellow-400 mt-1">{hoveredNode.name}</span>
          <p className="text-slate-300 text-[11px] leading-relaxed mt-0.5">{hoveredNode.description}</p>
          <div className="text-[10px] text-indigo-300 italic mt-1 border-t border-slate-800 pt-1">
            Proximity: walk near to initiate dialogue
          </div>
        </div>
      )}
    </div>
